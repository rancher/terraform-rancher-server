locals {
  ...
  elb_load_balancers      = var.use_alb_instead_of_elb ? null : [aws_elb.rancher[0].name]
  alb_target_group_arns = var.use_alb_instead_of_elb ? [aws_lb_target_group.rancher_alb[0].arn] : null
}
